...
class calc_allan:
    ...
    def plot_with_RRW(self,fig,ax):
        ...
        ax.loglog(tauz, adevz,label = r'$\sigma_Y$')
        ax.loglog(tauz, lineKz,linestyle='dashed',label = r'$\sigma_{Ky}$')
        ax.loglog(tauKz, Kz,label = r'$N_Y$',marker = 'o',color = 'g')

        taux,adevx,lineNx,tauNx,Nx,lineKx,tauKx,Kx,lineB,tauB,B,_ = self.a_varinace('gx')
        ax.loglog(taux, adevx,label = r'$\sigma_Y$')
        ax.loglog(taux, lineKx,linestyle='dashed',label = r'$\sigma_{Ky}$')
        ax.loglog(tauKx, Kx,label = r'$N_Y$',marker = 'o',color = 'r')
    ...
    def plot_BS(self,fid,ax):
        ...
        ax.loglog(tauy, adevy,label = r'$\sigma_Y$')
        ax.loglog(tauy, lineBy,linestyle='dashed',label = r'$\sigma_{Ny}$')
        ax.loglog(tauBy, scfby,label = r'$N_Y$',marker = 'o',color = 'b')

        tauz,adevz,lineNz,tauNz,Nz,lineKz,tauKz,Kz,lineBz,tauBz,Bz,scfbz = self.a_varinace('gz')
        ax.loglog(tauz, adevz,label = r'$\sigma_Z$')
        ax.loglog(tauz, lineBz,linestyle='dashed',label = r'$\sigma_{Nz}$')
        ax.loglog(tauBz, scfbz,label = r'$N_Z$',marker = 'o',color = 'g')

        taux,adevx,lineNx,tauNx,Nx,lineKx,tauKx,Kx,lineBy,tauBy,Bx,scfbx = self.a_varinace('gx')
        ax.loglog(taux, adevx,label = r'$\sigma_X$')
        ax.loglog(taux, lineBy,linestyle='dashed',label = r'$\sigma_{Nx}$')
        ax.loglog(tauBy, scfbx,label = r'$N_X$',marker = 'o',color = 'r')
    ...
    def plot_with_adev(self,fig,ax):
        tauz,adevz,lineNz,tauNz,Nz = self.a_varinace('gz')
        ax.loglog(tauz, adevz,label = 'Gyro-z')

        taup,pitch,_,_,_ = self.a_varinace('gy')
        ax.loglog(taup, pitch,label = 'Gyro-y')

        taur,roll,_,_,_ = self.a_varinace('gx')
    ...
    def get_gyros(self):
        ...
        data['gyro_x'] = data['gx']- data['gx'][0]
        data['gyro_y'] = data['gy']- data['gy'][0]
        data['gyro_z'] = data['gz']- data['gz'][0]
        ...
        plt.plot(data['time'], data['gyro_x'],label = 'Accel x(rad/s)')
        plt.plot(data['time'], data['gyro_y'],label = 'Accel y(rad/s)')
        plt.plot(data['time'], data['gyro_z'],label = 'Accel z(rad/s)')
        # plt.ylim(-0.035,0.035)
        plt.xlabel('Time (s)',fontsize=20)
        plt.ylabel('Accel (rad/s)',fontsize=20)
...
if __name__ == "__main__":
    loc_a = '/home/niket/n1_catkin_ws/src/data/LocationA.bag'
    gyro_csv = '/home/niket/n1_catkin_ws/src/data/LocationA/demo_gyro_D.csv'
    accel_csv = '/home/niket/n1_catkin_ws/src/data/LocationA/demo_accel_A.csv'
...
# 
    ...
    # adev_gyro.a_varinace('gy')
    # adev_accel.get_gyros(loc_a)

    # adev_gyro.a_varinace('gy')
    # ax.loglog(taup, pitch,label = 'Gyro-y')

    # taur,roll = adev_gyro.a_varinace('gx')
    # ax.loglog(taur, roll,label = 'Gyro-x')
